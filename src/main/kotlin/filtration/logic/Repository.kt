...
import filtration.github.APIExtractor
import filtration.github.GitHubAPI
import filtration.github.GraphQLQueries
import filtration.logic.filters.Filter
import filtration.logic.filters.FilterResult
import filtration.requests.getBody
import filtration.requests.getRequest
import filtration.requests.isOK
import filtration.requests.postRequest
import utils.Logger
...
class Repository(
    ...
    fun loadCore(config: FilterConfig, limits: RateLimits, objectMapper: ObjectMapper): Boolean {
        var isCoreGood = true
    ...
    fun loadGraphQL(config: FilterConfig, limits: RateLimits, objectMapper: ObjectMapper): Boolean {
    ...
    fun toJSONExplain(objectMapper: ObjectMapper? = null): JsonNode {
        ...
        for (result in filterResults) {
            node.set<JsonNode>(result.field.configName, result.toJSON(objectMapper))
        }
        return node
    }
